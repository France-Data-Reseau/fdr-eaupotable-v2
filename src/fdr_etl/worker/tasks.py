...
def run_validation_task(self, filepath: str):
    ...
    logger.info(
        f"Validation réussie (Niveau de permissivité : {validation_res.get('level')})."
    )
    ...
    return {
        ...
        "message": "Fichier conforme avec avertissements."
        if global_status == "warning"
        else "Fichier conforme, prêt pour l'intégration.",
        "filepath": filepath,
...
def run_integration_task(
    ...
    logger.info(
        f"Chargement en base pour : {filepath} (ID: {import_id}) avec le niveau de permissivité: {permissivity}"
    )
    try:
        load_file_to_db(
            filepath=filepath,
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Les transformations ont échoué : {str(e)}",
        }
