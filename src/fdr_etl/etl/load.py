...
def load_file_to_db(filepath: str, db_url: str):
    ...
    """
    print(f"[Load] Chargement des données de {filepath} en base de données.")
    ...
    try:
        # On suppose que la table 'eaupotable' existe puisque validation ok.
        sqlite_cursor.execute("SELECT A, B, C FROM eaupotable")
        rows = sqlite_cursor.fetchall()
        print(f"       -> {len(rows)} lignes lues depuis le fichier geopackage.")
    except Exception as e:
        print(f"[Load] Erreur lors de la lecture SQLite: {e}")
        sqlite_conn.close()
        raise e
    ...
    try:
        with psycopg.connect(db_url) as pg_conn:
            with pg_conn.cursor() as pg_cursor:
                # Création de la table si elle n'existe pas (A, B, C sont de type INTEGER/NUMERIC)
                pg_cursor.execute("""
                ...
                if rows:
                    with pg_cursor.copy("COPY eaupotable (A, B, C) FROM STDIN") as copy:
                        for row in rows:
                            copy.write_row(row)
                    print(
                        f"[Load] Intégration de {len(rows)} lignes validée dans Postgres."
                    )

            # Commit implicite en sortant du bloc 'with pg_conn'
    except Exception as e:
        print(f"[Load] Erreur lors de l'intégration Postgres: {e}")
        raise e
