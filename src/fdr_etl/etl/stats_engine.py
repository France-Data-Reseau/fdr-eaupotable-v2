...
def execute_statistical_pipeline(
    df_source: pd.DataFrame, nom_coll: str, nb_annees: float
):
    if df_source.empty:
    ...
    logger.info(
        f"[{nom_coll}] Démarrage du pipeline statistique (Taille : {len(df_source)} lignes)"
    )
    ...
    df_source["annee_pose_brute"] = pd.to_numeric(
        df_source["date_pose"], errors="coerce"
    )
    ...
    # Âge moyen pondéré
    age_moyen = (
        (df_source["age_impute"] * df_source["longueur_km"]).sum() / total_km
        if total_km > 0
        else 0.0
    )
    ...
    taux_global_an = (
        (total_casses / total_km) / nb_annees if total_km > 0 and nb_annees > 0 else 0.0
    )
    ...
    logger.info(
        f"[{nom_coll}] Étape 1 (Calculs préliminaires) terminée en {time.time() - start_step:.3f}s"
    )
    ...
    logger.info(
        f"[{nom_coll}] Étape 2 (Matériau dominant) terminée en {time.time() - start_step:.3f}s"
    )

    # BOOTSTRAP + PATRIMOINE combinés
    start_bootstrap_global = time.time()
    mapping_types = {
        "materiau": "rep_materiau",
        "dia_ens": "rep_dia_ens",
        "ddp_ens": "rep_ddp_ens",
    }
    all_stats = []
    ...
    for col, key in mapping_types.items():
        ...
        temp = (
            df_source.groupby(col, observed=False)["longueur_km"]
            .sum()
            .reset_index()
            .rename(columns={col: "categorie", "longueur_km": "km"})
        )
        ...
        logger.info(
            f"[{nom_coll}] Bootstrap pour '{col}' terminé en {time.time() - start_col_bootstrap:.3f}s"
        )

    logger.info(
        f"[{nom_coll}] Étape 3 (Total des 3 Bootstraps) terminée en {time.time() - start_bootstrap_global:.3f}s"
    )
    ...
    if not df_stats.empty:
        ...
        df_patrimoine = df_patrimoine.merge(
            df_stats, on=["analyse_type", "categorie"], how="left"
        )
    else:
    ...
    df_patrimoine[["ic_inf", "ic_sup", "taux_moyen"]] = df_patrimoine[
        ["ic_inf", "ic_sup", "taux_moyen"]
    ].fillna(0.0)
    df_patrimoine.replace([np.inf, -np.inf], 0.0, inplace=True)

    # Sortie croisée pour la consolidation nationale (Etape 5)
    df_croise = (
        df_source.groupby(["materiau", "dia_ens", "ddp_ens"], observed=False)
        .agg({"longueur_km": "sum", "nb_casses": "sum"})
        .reset_index()
    )
    ...
    logger.info(
        f"[{nom_coll}] Étape 4 & 5 (Assemblage et croisement) terminés en {time.time() - start_step:.3f}s"
    )
    logger.info(
        f"[{nom_coll}] Pipeline TOTAL exécuté en {time.time() - pipeline_start:.3f}s"
    )
