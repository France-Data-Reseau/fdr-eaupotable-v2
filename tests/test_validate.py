...
def test_validate_file_success(temp_gpkg):
    ...
    conn.execute("CREATE TABLE eaupotable (fid INTEGER, A NUMERIC, B NUMERIC, C NUMERIC)")
...
def test_validate_file_wrong_types(temp_gpkg):
    """Teste l'échec si une colonne contient un type invalide (string au lieu de number)."""
    conn = sqlite3.connect(temp_gpkg)
    conn.execute("CREATE TABLE eaupotable (fid INTEGER, A NUMERIC, B NUMERIC, C NUMERIC)")
